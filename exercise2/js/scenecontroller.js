...
SceneController.prototype.setupGUI = function()
{
    this.otherParams = {
        clipAxes: false,
        sceneAxes: false,
        enableSceneOrbit: false,
        enableClipOrbit: true,
        enableOrbitInClip: false,
    ...
    this.gui.add( this.otherParams, "sceneAxes" ).name("World Axis");
    this.gui.add( this.otherParams, "clipAxes" ).name("Clipping Axis");
    this.gui.add( this.otherParams, "enableSceneOrbit" ).name("Scene Orbit");
    this.gui.add( this.otherParams, "enableClipping" ).name("Enable Clipping");
    this.gui.add( this.otherParams, "enableOrbitInClip" ).name("Clip Orbit");
...
SceneController.prototype.adjustClipView = function()
{
    ...
    var newBear = createTeddyBear(this.params);
    newBear.updateMatrix();
    newBear.updateMatrixWorld();

    var childNodes = []
    childNodes.push(newBear.children[0]);
    while (childNodes.length > 0) {
        var childNode = childNodes.pop();
        if (childNode.children.length > 0) {
            childNodes.push.apply(childNodes, childNode.children);
        }

        if (childNode.geometry != undefined) {
            childNode.updateMatrixWorld();
            childNode.geometry.mergeVertices();
            childNode.geometry.verticesNeedUpdate = true;
            for (var i = 0; i < childNode.geometry.vertices.length; i++) {
                var tempVertex = new THREE.Vector4;
                tempVertex.copy(childNode.geometry.vertices[i]);
                tempVertex.applyMatrix4(childNode.matrixWorld);
                tempVertex.applyMatrix4(this.perspectiveCamera.matrixWorldInverse);
                tempVertex.applyMatrix4(this.perspectiveCamera.projectionMatrix);
                tempVertex.divideScalar(tempVertex.w);
                tempVertex.z *= -1;
                childNode.geometry.vertices[i].copy(tempVertex);
            }
        }
    }
    this.bear2 = newBear;
...
SceneController.prototype.render = function()
{
    ...
    this.clipRenderer.localClippingEnabled = this.otherParams.enableClipping;
    this.screenRenderer.localClippingEnabled = this.otherParams.enableClipping;
...
SceneController.prototype.animate = function()
{
    ...
    this.clipControls.enabled = this.otherParams.enableOrbitInClip;
